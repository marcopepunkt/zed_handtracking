...
NUM_BLOBS = 3
color_dict = {39725782: (255, 0, 0), 38580376: (0, 255, 0), 0: (0, 0, 0)}
...
class Hand: 
    ...
    def get_hand_pose(self, xyzs):
        """We get the hand pose from the xyz coordinates of the markers"""
        # Do I need some kalman filter, first try the stupid approach
        # Do the cross product of the positions to get a homogenous transformation of the hand for visualization
        pass
    ...
    while loop:
        ...
        for cam in cams:
            ...
            if len(blob_centers) != 0:
                ...
                # For the first detected frame, we need to define the marker IDs 
                # Marker 1 is thumb, Marker 2 is index finger base and Marker 3 is index finger tip
                # Marker 1 is the furthest to the right, then 2 is the furthest up and 3 is the furthest down 
                # if first_frame: 
                #   hand = Hand.assign_markers(matched_uvs)
                ...
                # Then after all the ones are matched, we can get the hand
                # hand_pose = hand.get_hand_pose(matched_xyzs)
                # Then we can visualize the hand pose 
        ...
        if len(matched_blobs_uv) ==2: 
            mock_data = [[(cams[0].width // 2, cams[0].height // 2)], [(cams[1].width // 2, cams[1].height // 2)]]
...
if __name__ == "__main__":  
    ...
    # cv2.createTrackbar("cam0_horizontal", "Tracking", 0, width, nothing)
    # cv2.createTrackbar("cam0_vertical", "Tracking", 0, height, nothing)
    # cv2.createTrackbar("cam1_horizontal", "Tracking", 0, width, nothing)
    # cv2.createTrackbar("cam1_vertical", "Tracking", 0, height, nothing)

    # while True:
    #     cam0_horizontal = cv2.getTrackbarPos("cam0_horizontal", "Tracking")
    #     cam0_vertical = cv2.getTrackbarPos("cam0_vertical", "Tracking")
    #     cam1_horizontal = cv2.getTrackbarPos("cam1_horizontal", "Tracking")
    #     cam1_vertical = cv2.getTrackbarPos("cam1_vertical", "Tracking")
    #     center0_changed = [cam0_horizontal, cam0_vertical]
    #     center1_changed = [cam1_horizontal, cam1_vertical]
    #     points = triangulation(cams, [center0_changed, center1_changed])
    #     print(points)
    #     vis.visualize_points(-points, color = (0, 0, 0))
